...
```{r}
library(stringr)
episode_no <- nrow(season_data)
date_start <- str_sub(season_data$premiere_date, -11, -2)[1]
date_end <- str_sub(season_data$premiere_date, -11, -2)[episode_no]
avg_viewers <- mean(season_data$viewers)
```

Season r parmas\$season of Game of Thrones consisted of episode_no episodes that aired between r date_start and r date_end on HBO. The show gathered an average of r avg_viewers first-day TV viewers in the US, with the number growing from 2.22 to 3.04 million by the end of the season.
